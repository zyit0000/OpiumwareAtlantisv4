...
// Opiumware port state
static ACTIVE_PORT: Lazy<Mutex<Option<u16>>> = Lazy::new(|| Mutex::new(None));
const OPIUM_PORTS: &[u16] = &[8392, 8393, 8394, 8395, 8396, 8397];
#[tauri::command]
fn get_simple_username() -> Result<String, String> {
    Ok("ZYiT0".to_string())
}

#[tauri::command]
fn get_computer_name() -> Result<String, String> {
    Ok("Ken Carson".to_string())
}
#[derive(Clone)]
struct ProcessInfo {
    pid: u32,
...
#[tauri::command]
fn get_desktop_username() -> Result<UserInfo, String> {
...
async fn attach() -> Result<String, String> {
    for &port in OPIUM_PORTS {
        match TcpStream::connect_timeout(
            ...
            Ok(_) => {
                let mut active = ACTIVE_PORT.lock().unwrap();
                *active = Some(port);
                return Ok(format!("Successfully attached to port: {}", port));
            }
            Err(_) => continue,
        }
    }
    Err("Failed to connect to Opiumware on all ports (8392-8397)".to_string())
}

#[tauri::command]
async fn get_key() -> Result<(), String> {
    let url = "https://discord.gg/opiumware";
    match open::that(url) {
        Ok(_) => Ok(()),
...
// Prepend Opiumware prefix and prepare script
fn prepare_script(user_script: &str) -> String {
    format!("OpiumwareScript {}", user_script)
}
...
async fn execute(lua_code: String) -> Result<String, String> {
    ...
    let port = {
        let active = ACTIVE_PORT.lock().unwrap();
        active.ok_or_else(|| "Not attached. Please click Inject first!".to_string())?
    };

    let prefixed_code = prepare_script(&lua_code);
    
    // Zlib compression (deflate)
    let mut encoder = ZlibEncoder::new(Vec::new(), Compression::default());
    encoder.write_all(prefixed_code.as_bytes()).map_err(|e| e.to_string())?;
    ...
    match TcpStream::connect_timeout(
        ...
        Ok(mut stream) => {
            stream.write_all(&compressed_data).map_err(|e| e.to_string())?;
            stream.flush().map_err(|e| e.to_string())?;
            Ok(format!("Successfully executed to port: {}", port))
        }
        Err(e) => Err(format!("❌ Connection failed to port {}: {}", port, e))
    }
}
